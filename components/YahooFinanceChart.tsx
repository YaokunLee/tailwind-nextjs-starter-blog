...
const YahooFinanceChart: React.FC<YahooFinanceChartProps> = ({
  ...
  const fetchAlphaVantageData = useCallback(
    async (symbol: string): Promise<StockData[]> => {
      try {
        ...
        Object.entries(timeSeries).forEach(([date, values]: [string, Record<string, string>]) => {
          stockData.push({
            date,
  ...
  const drawChart = useCallback(() => {
    ...
    if (visibleData.length > 0) {
      ...
      // 第二行：数据日期和成交量
      ctx.fillText(
        `日期: ${latestData.date} | 成交量: ${(latestData.volume / 1000000).toFixed(1)}M`,
        10,
        canvas.height - 15
      )
    }
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent) => {
      ...
      const newVisible = Math.min(
        stockData.length,
        Math.max(10, Math.floor(currentVisible * zoomFactor))
      )
  ...
  if (loading) {
    return (
      <div className="my-6 overflow-hidden rounded-lg border border-blue-200 dark:border-blue-700">
        <div
        ...
        >
          <div className="text-center">
            ...
            <p className="font-medium text-blue-600">🌐 正在获取 {symbol} 真实股价数据...</p>
            <p className="mt-1 text-sm text-blue-500">连接Yahoo Finance API</p>
          </div>
        </div>
      </div>
    )
  ...
  if (error) {
    return (
      <div className="my-6 overflow-hidden rounded-lg border border-red-200 dark:border-red-700">
        ...
        >
          <div className="text-center">
            <p className="mb-2 font-medium text-red-600">❌ 真实数据获取失败</p>
  ...
  return (
    <div className="my-6 overflow-hidden rounded-lg border border-green-200 dark:border-green-700">
      {/* 控制面板 */}
      <div className="border-b border-green-200 bg-green-50 p-4 dark:border-green-700 dark:bg-green-900">
        <div className="mb-2 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-green-600 dark:text-green-400">
              显示 {chartState.viewportEnd - chartState.viewportStart} / {stockData.length}{' '}
              天真实交易数据
            </p>
          </div>
          <button
      ...
      <div className="bg-green-50 p-2 text-center text-xs text-green-600 dark:bg-green-900">
